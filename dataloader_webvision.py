    ...
    def __getitem__(self, index):
        if self.mode=='labeled':
            ...
            if index in self.pred_idx:
                ...
                pred_clean = 0   
            return img, target, pred_clean, index      
        elif self.mode=='test':
...
class webvision_dataloader():  
    ...
    def run(self,mode,pred=[],prob=[]):
        if mode=='warmup':
            all_dataset = webvision_dataset(root_dir=self.root_dir, transform=self.transform_train, mode="warmup", num_class=self.num_class)                
            ...
            all_dataset = webvision_dataset(root_dir=self.root_dir, transform=self.transform_train, mode="all", num_class=self.num_class, pred=pred,probability=prob)                
            trainloader = DataLoader(
                dataset=all_dataset, 
            ...
            eval_dataset = webvision_dataset(root_dir=self.root_dir, transform=self.transform_test, mode='warmup', num_class=self.num_class)      
...
class webvision_ddp_dataloader():  
    def __init__(self, batch_size, num_class, num_workers, root_dir, log):
    ...
    def run(self,mode,pred=[],prob=[]):
        if mode=='warmup':
            ...
            labeled_dataset = webvision_dataset(root_dir=self.root_dir, transform=self.transform_train, mode="labeled",num_class=self.num_class,pred=pred,probability=prob,log=self.log)              
            labeled_train_sampler = torch.utils.data.distributed.DistributedSampler(labeled_dataset)
            labeled_trainloader = DataLoader(
                dataset=labeled_dataset, 
                batch_size=self.batch_size,
                num_workers=self.num_workers,
            ...
            unlabeled_dataset = webvision_dataset(root_dir=self.root_dir, transform=self.transform_train, mode="unlabeled",num_class=self.num_class,pred=pred,log=self.log)                    
            unlabeled_train_sampler = torch.utils.data.distributed.DistributedSampler(unlabeled_dataset)
            unlabeled_trainloader = DataLoader(
                dataset=unlabeled_dataset, 
                batch_size=self.batch_size,
                num_workers=self.num_workers,
