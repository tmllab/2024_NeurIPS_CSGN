...
if not os.path.exists('./pretrains/'+args.dataset):
    os.system('mkdir -p %s'%('./pretrains/'+args.dataset))
...
torch.save({
    ...
    }, './pretrains/%s/checkpoint_%s_%.2f'%(args.dataset, args.noise_mode, args.r)+'.tar')
